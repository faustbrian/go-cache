...
- no direct dependency on `go-queue`

### Observability

- hit, miss, stale, load, error, latency, eviction, and size metrics
- OpenTelemetry integration through optional `go-telemetry` adapters
- optional `go-log`/`slog` hooks with mandatory key/value redaction controls
