...
if redis.call('EXISTS', KEYS[2]) == 0 or
   redis.call('HGET', KEYS[2], 'owner') ~= ARGV[1] or
   redis.call('HGET', KEYS[2], 'token') ~= ARGV[2] then
  return 0
end
...
// SetIfOwned atomically compares an active Valkey lease hash and publishes the
// record. A missing, expired, or superseded lease returns ErrOwnershipLost.
func (b *Backend) SetIfOwned(
	...
	command := b.client.B().Eval().Script(protectedSet).Numkeys(2).
		Key(key).Key(guard.StorageKey()).
		Arg(guard.Owner()).Arg(guard.Token()).Arg(value).
...
func (b *Backend) encode(record cache.Record) (string, time.Duration, error) {
	...
	return valkeyclient.BinaryString(encoded), ttl, nil
}
