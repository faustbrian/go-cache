...
| `SetIfOwned` | Optional atomic active-owner comparison and write; unsupported backends fail closed |
...
Successful `Set`, `SetIfOwned`, `Add`, `Replace`, or `Delete` on the same
`Cache` instance wins over an active load. Failed and rejected conditional
mutations do not supersede it. Ordinary mutation ordering is process-local;
the optional Valkey ownership check covers protected refresh publication only.
...
| Atomic ownership-guarded publish | Unsupported | Unsupported | Lua owner/token comparison |
...
| Invalidation race | Same-instance mutations supersede active loads; Valkey protected refresh can require an active lease | Ordinary cross-process invalidation remains unfenced; use source versions or versioned keys when ordering is critical |
...
| H-02 | High | Validate-then-set across separate Valkey calls allows an expired predecessor to publish after a successor | Fixed with one Lua owner/token comparison and cache write; stale, missing, outage, and local-load precedence regressions added |
...
**Verdict: PASS for the audited standalone scope.** The initial release audit
passed the following commands on 2026-07-15:
